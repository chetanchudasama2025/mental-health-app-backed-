...
if (!accountSid || !authToken || !phoneNumber) {
  console.warn('Twilio credentials are not fully configured. Phone verification will not work.');
}
...
export const sendOTP = async (to: string, otp: string): Promise<void> => {
  ...
  try {
    await twilioClient.messages.create({
      body: `Your Mental Health App verification code is: ${otp}. This code will expire in 10 minutes.`,
      from: phoneNumber,
      to: to,
    });
  } catch (error) {

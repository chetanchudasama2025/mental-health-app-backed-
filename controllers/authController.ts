...
const JWT_REMEMBER_REFRESH_EXPIRES_IN = process.env.JWT_REMEMBER_REFRESH_EXPIRES_IN || "90d";
...
const generateToken = (userId: string, tokenVersion: number, expiresIn?: string): string => {
...
const generateRefreshToken = (userId: string, tokenVersion: number, expiresIn?: string): string => {
    const secret = JWT_SECRET;
    if (!secret) {
        throw new Error("JWT_SECRET is not defined");
    }
...
export const login = async (
    ...
    try {
        ...
        if (userWithDeleted && userWithDeleted.deletedAt) {
            const error: CustomError = new Error(
                "This account is currently deleted. If you would like to restore access, please contact the support team."
            );
        ...
        const tokenExpiresIn = rememberMe ? JWT_REMEMBER_EXPIRES_IN : JWT_EXPIRES_IN;
        const refreshTokenExpiresIn = rememberMe ? JWT_REMEMBER_REFRESH_EXPIRES_IN : JWT_REFRESH_EXPIRES_IN;
...
export const sendVerificationEmailAPI = async (
    ...
    try {
        ...
        let attemptsToday = 1;
        const isSameDay =
            existingToken?.lastAttemptDate &&
            new Date(existingToken.lastAttemptDate).toDateString() ===
            today.toDateString();

        if (isSameDay && existingToken) {
            attemptsToday = (existingToken.attemptsToday || 0) + 1;
        }
...
export const sendVerificationPhoneAPI = async (
    ...
    try {
        ...
        const existingToken = await PhoneVerificationToken.findOne({
            phone: phoneNumber,
            deletedAt: null,
        });
...
export const refreshToken = async (
    ...
    try {
        ...
        const tokenExpiresIn = user.isRemember ? JWT_REMEMBER_EXPIRES_IN : JWT_EXPIRES_IN;
        const refreshTokenExpiresIn = user.isRemember ? JWT_REMEMBER_REFRESH_EXPIRES_IN : JWT_REFRESH_EXPIRES_IN;
...
export const googleCallback = async (
    ...
    try {
        ...
        if (user && user.deletedAt) {
            return res.redirect(
                `${process.env.FRONTEND_URL}/auth/login?error=account_deleted&message=This account is currently deleted. If you would like to restore access, please contact the support team.`
            );
        }
